...
const LicService = require('../service/lic');
const Validator = require('../utils/validator');
const { save: saveSchema, patch: patchSchema } = require('../dto-schemas/lic');
...
const save = async (req, res) => {
  try {
    ...
    if (customerId) {
      return res.status(400).json({ status: 'error', message: 'Relative cannot save the customer lic' });
    }
    ...
    const { errors: serviceErrors, doc } = await LicService.save(data);
...
const getAll = async (req, res) => {
  try {
    ...
    const { count, doc } = await LicService.getAll({ userId, customerId });
...
const patch = async (req, res) => {
  try {
    ...
    if (customerId) {
      return res.status(400).json({ status: 'error', message: 'Relative cannot edit the customer lic' });
    }
    ...
    const { errors: err, doc } = await LicService.patch(data);
...
const deleted = async (req, res) => {
  try {
    ...
    const { errors: err, doc } = await LicService.deleted(data);
