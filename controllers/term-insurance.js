...
const termInsuranceService = require('../service/term-insurance');
const Validator = require('../utils/validator');
const { save: saveSchema, patch: patchSchema } = require('../dto-schemas/term-insurance');
...
const save = async (req, res) => {
  try {
    ...
    if (customerId) {
      return res.status(400).json({ status: 'error', message: 'Relative cannot save the customer term insurance' });
    }
    ...
    const { errors: serviceErrors, doc } = await termInsuranceService.save(data);
...
const getAll = async (req, res) => {
  try {
    ...
    const { count, doc } = await termInsuranceService.getAll({ userId, customerId });
...
const patch = async (req, res) => {
  try {
    ...
    if (customerId) {
      return res.status(400).json({ status: 'error', message: 'Relative cannot edit the customer term insurance' });
    }
...
const deleted = async (req, res) => {
  try {
    ...
    if (customerId) {
      return res.status(400).json({ status: 'error', message: 'Relative can not delete the customer term insurance' });
    }
