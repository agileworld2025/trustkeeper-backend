...
const save = async (req, res) => {
  try {
    ...
    if (customerId) {
      return res.status(400).json({ status: 'error', message: 'Relative cannot save the customer credit card' });
    }
    ...
    const { errors: serviceErrors, doc } = await CreditCardService.save(data);
...
const getAll = async (req, res) => {
  try {
    ...
    const { count, doc } = await CreditCardService.getAll({ userId, customerId });
...
const patch = async (req, res) => {
  try {
    ...
    if (customerId) {
      return res.status(400).json({ status: 'error', message: 'Relative cannot edit the customer credit card' });
    }
