...
const encryptionFields = encryptionConfig['fixed-deposit'] || [];
...
const save = async (data) => {
  try {
    ...
    return { doc: { publicId, message: 'fixed deposit details successfully saved.' } };
  } catch (error) {
    return { errors: [ { name: 'save', message: 'An error occurred while saving fixed deposit data' } ] };
  }
};
...
const getAll = async (payload) => {
  try {
    ...
    const response = await FixedDepositModel.findAll({
    ...
    return { errors: [ { name: 'getAll', message: 'An error occurred while fetching fixed deposit data' } ] };
  }
};
...
const patch = async (payload) => {
  try {
    ...
    const [ updatedCount ] = await FixedDepositModel.update(updateData, {
      where: { public_id: publicId, is_deleted: false },
    });

    if (!updatedCount) {
      return { errors: [ { name: 'patch', message: 'No fixed deposit record found' } ] };
    }

    return { doc: { message: 'fixed deposit details successfully updated.', publicId } };
  } catch (error) {
    return { errors: [ { name: 'patch', message: 'An error occurred while updating fixed deposit data' } ] };
  }
};
...
const deleted = async (payload) => {
  try {
    ...
    const [ updatedCount ] = await FixedDepositModel.update(
      { is_deleted: true, updated_by: updatedBy },
      { where: { public_id: publicId, is_deleted: false } },
    );

    if (!updatedCount) {
      return { errors: [ { name: 'deleted', message: 'No fixed deposit record found' } ] };
    }

    return { doc: { message: 'fixed deposit details successfully deleted.' } };
  } catch (error) {
    return { errors: [ { name: 'deleted', message: 'An error occurred while deleting fixed deposit data' } ] };
  }
};
