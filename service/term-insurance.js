...
const { term_insurance: termInsuranceModel, sequelize } = require('../database');
...
const getAll = async (payload) => {
  ...
  const response = await termInsuranceModel.findAll({
...
const patch = async (payload) => {
  ...
  try {
    const response = await termInsuranceModel.findOne({
    ...
    await termInsuranceModel.update(
      {
...
const deleted = async (payload) => {
  ...
  const res = await termInsuranceModel.findOne({
    where: { public_id: publicId },
  });
  ...
  if (res) {
    ...
    await termInsuranceModel.update({ is_deleted: true, updated_by: updatedBy }, { where: { public_id: publicId } });
